...
hts.Consented,
hts.TestedAs,
CASE WHEN hts.Test_1_Final_Result = 'Positive' AND ROW_NUMBER() OVER (PARTITION BY hts.Person_Id ORDER BY hts.Encounter_ID asc) > 1 THEN 'Repeat Test' ELSE hts.TestType END AS TestType,
hts.Test_1_Kit_Name,
...
) PPR2 ON PPR2.Person_Id = P.Id) hts
...
WHERE htsp.Test_1_Final_Result = 'Positive' and htsp.Person_Id not in (
select 
htspr.Person_Id
FROM(SELECT 
HE.PersonId Person_Id,
ROW_NUMBER() OVER (PARTITION BY HE.PersonId ORDER BY HE.Id asc) as rownum
...
) PPR2 ON PPR2.Person_Id = P.Id) htspr
