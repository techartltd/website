SELECT P.PersonId Person_Id,
	...
	WHERE ov.VisitType = 4 AND o.DispensedByDate IS NOT NULL AND o.Ptn_pk = P.ptn_pk and o.DispensedByDate = PM.VisitDate)
...
CASE WHEN MP.Ptn_Pk is null THEN P.ptn_pk else MP.Ptn_Pk end as Ptn_Pk,
...
LEFT JOIN (
		...
		WHERE a.deleteflag = 0 AND a.ModuleId IN(SELECT m.ModuleID FROM mst_module m WHERE m.ModuleName='CCC Patient Card MoH 257')) bapp ON bapp.Visit_pk = OV.Visit_Id
...
P.PersonId Person_Id,
P.Ptn_Pk,
format(cast(t.VisitDate AS DATE), 'yyyy-MM-dd') AS Encounter_Date,
NULL AS Encounter_ID,
...
from(SELECT dateadd(day, d.Duration, o.DispensedByDate) AS ExpectedReturn, o.Ptn_pk, ov.VisitDate, ov.VisitType
FROM ord_PatientPharmacyOrder o
INNER JOIN ord_Visit ov ON ov.Visit_Id = o.VisitID
INNER JOIN dtl_PatientPharmacyOrder d ON d.ptn_pharmacy_pk = o.ptn_pharmacy_pk
WHERE ov.VisitType = 4 AND o.DispensedByDate IS NOT NULL) t
...
P.PersonId Person_Id,
P.Ptn_Pk,
format(cast(PD.VisitDate AS DATE), 'yyyy-MM-dd') AS Encounter_Date,
NULL AS Encounter_ID,
NULL AS Visit_scheduled,
NULL AS Visit_by,
NULL Visit_by_other,
...
WHERE o.DispensedByDate is not null AND PE.EncounterTypeId = (SELECT ItemId FROM LookupItemView WHERE MasterName = 'EncounterType' AND ItemName = 'Pharmacy-encounter')) PD
LEFT JOIN (
SELECT 
PM.VisitDate,
PM.PatientId
